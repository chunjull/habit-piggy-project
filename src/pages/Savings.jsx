  ...
  const renderUncompletedHabits = (habits, startOfPeriod, endOfPeriod) => {
    ...
    return uncompletedHabits.slice(0, 50).map((habit, index) => (
      <li key={habit.id + habit.date} className="py-2 px-4 grid grid-cols-5 bg-black-50 dark:bg-black-800 rounded-lg font-normal text-sm leading-5 hover:bg-black-0 dark:hover:bg-black-600">
        ...
        <p className="text-center text-black dark:text-black-0">{new Date(habit.date).toLocaleDateString()}</p>
        <p className="text-center col-span-2 overflow-scroll text-black dark:text-black-0">{habit.title}</p>
        <p className="text-center text-black dark:text-black-0">NT${habit.amount}</p>
      </li>
    ));
  };
    ...
    <div className="p-4 md:py-10 space-y-4">
      ...
      {isActiveTab === "overview" && (
        <div>
          ...
          <p className="text-center mt-2 font-normal text-xs leading-4 text-black dark:text-black-0">僅顯示最新的 50 筆存款記錄</p>
        </div>
      )}
      {isActiveTab === "category" && (
        <div className="p-4 bg-black-50 dark:bg-black-800 rounded-xl space-y-4 min-h-screen">
          ...
          {savingsCount === 0 ? (
            <p className="text-black dark:text-black-0">查無資料</p>
          ) : (
            <div>
